<template>
  <div class="min-h-screen">
    <!-- Header -->
    <section class="relative py-24 bg-gradient-to-b from-amber-50 to-white overflow-hidden">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="max-w-3xl mx-auto text-center">
          <span class="text-amber-600 font-semibold text-sm uppercase tracking-wider">{{ pageContent.header.badge || 'Nuestra Esencia' }}</span>
          <h1 class="text-4xl md:text-5xl font-serif font-bold text-earth-900 mt-3 mb-6">
            {{ pageContent.header.title }} <span class="text-transparent bg-clip-text bg-gradient-to-r from-amber-500 to-primary-600">{{ pageContent.header.highlight }}</span>
          </h1>
    ...
    <section class="py-20">
      <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-3xl border border-earth-100 p-8 md:p-12 shadow-sm relative">
          <!-- Sello visual ecológico -->
          <div class="absolute -top-6 -right-6 w-32 h-32 md:w-40 md:h-40 rounded-full overflow-hidden shadow-lg border-4 border-white hidden md:block">
            <img
              :src="pageContent.manifesto.image_url"
              :alt="pageContent.manifesto.image_alt"
              class="w-full h-full object-cover"
              loading="lazy"
            />
          </div>
          
          <h2 class="text-2xl md:text-3xl font-serif font-bold text-earth-900 mb-8 text-center">
            🌿 Manifiesto PITAYA LAB
          </h2>
          
          <div class="space-y-6 text-earth-600 leading-relaxed">
            <div v-for="(item, index) in manifesto" :key="index" class="flex gap-4">
              <span class="text-2xl flex-shrink-0">{{ item.icon }}</span>
              <div>
                <h3 class="font-semibold text-earth-800 mb-1">{{ item.title }}</h3>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div class="relative order-2 lg:order-1">
            <div class="aspect-square rounded-3xl overflow-hidden shadow-2xl">
              <img
                :src="pageContent.eco.image_url"
                :alt="pageContent.eco.image_alt"
                class="w-full h-full object-cover"
                loading="lazy"
              />
            </div>
          ...
          <div class="order-1 lg:order-2">
            <span class="text-primary-600 font-semibold text-sm uppercase tracking-wider">{{ pageContent.eco.badge }}</span>
            <h2 class="text-3xl md:text-4xl font-serif font-bold text-earth-900 mt-2 mb-6">
              {{ pageContent.eco.title }}
            </h2>
            <div class="space-y-4 text-earth-600">
              <div class="space-y-4 text-earth-600 leading-relaxed" v-html="ecoIntroHtml"></div>
              <ul class="space-y-3">
                <li v-for="(item, index) in pageContent.eco.commitments" :key="index" class="flex items-start gap-3">
              ...
              <p class="mt-6" v-html="pageContent.eco.outro"></p>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- CTA -->
    <section class="py-20 bg-gradient-to-r from-primary-50 to-amber-50">
      <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h2 class="text-3xl font-serif font-bold text-earth-900 mb-4">
        ...
        <a
          ...
          class="inline-flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white px-8 py-4 rounded-full text-lg font-semibold transition-all hover:shadow-xl hover:shadow-amber-200"
        >
          ...
          {{ pageContent.cta.button_text || 'Comprar en Amazon' }}
        </a>
      </div>
    </section>
  </div>
...
useSeoMeta({
  ...
  ogDescription: 'Manifiesto y compromiso ambiental de PITAYA LAB. Productos biodegradables para un hogar consciente.',
})

const supabase = useNuxtApp().$supabase
const AMAZON_LINK = 'https://www.amazon.com.mx/stores/PitayaLab/page/9A7C33BA-7EBF-41E8-9F0F-FEE7FE78A329?'

// Contenido editable desde el panel admin vía site_config
const pageContent = reactive({
  ...
  eco: {
    ...
    intro: 'Cada decisión que tomamos en <strong class="text-earth-800">PITAYA LAB</strong> está guiada por nuestro compromiso con el medio ambiente.',
    commitments: [
...
const manifesto = computed(() => pageContent.manifesto.items)

const ecoIntroHtml = computed(() => `<p>${pageContent.eco.intro}</p>`)
...
async function loadConfig() {
  ...
  try {
    const { data, error } = await supabase
      .from('site_config')
      .select('value')
      .eq('key', 'philosophy_page')
      .single()
    if (error) throw error
    if (data?.value) {
      if (data.value.header) Object.assign(pageContent.header, data.value.header)
      if (data.value.manifesto) Object.assign(pageContent.manifesto, data.value.manifesto)
      if (data.value.eco) Object.assign(pageContent.eco, data.value.eco)
      if (data.value.cta) Object.assign(pageContent.cta, data.value.cta)
    }
  } catch (e) {
    console.warn('Usando datos estáticos para Philosophy (fallback):', e.message)
  }
}
