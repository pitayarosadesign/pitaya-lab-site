<template>
  <div class="min-h-screen">
    ...
    <section class="relative py-24 bg-gradient-to-b from-amber-50 to-white overflow-hidden">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="max-w-3xl mx-auto text-center">
          <span class="text-amber-600 font-semibold text-sm uppercase tracking-wider">{{ pageContent.header.badge || 'Ayuda' }}</span>
    ...
    <!-- Destacado Prime -->
    <section v-if="pageContent.prime_banner.enabled" class="-mt-8 relative z-10">
      <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-gradient-to-r from-amber-500 to-amber-600 rounded-2xl shadow-xl p-6 md:p-8 text-center">
          <div class="flex items-center justify-center gap-3 mb-2">
            ...
            <h2 class="text-2xl md:text-3xl font-bold text-white">{{ pageContent.prime_banner.title }}</h2>
          </div>
          <p class="text-amber-100 text-sm md:text-base" v-html="pageContent.prime_banner.text"></p>
        </div>
      </div>
    </section>
      ...
      <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="space-y-4">
          <div
            ...
            :key="index"
            class="bg-white rounded-2xl border border-earth-100 overflow-hidden transition-all hover:shadow-sm"
          >
            <button
              @click="toggleFaq(index)"
              class="w-full flex items-center justify-between p-6 text-left"
            >
              <span class="text-base md:text-lg font-semibold text-earth-800 pr-4">{{ faq.question }}</span>
              <svg
                :class="openFaqs.includes(index) ? 'rotate-180 text-primary-600' : 'text-earth-400'"
            ...
            <div
              ...
              class="px-6 pb-6"
            >
              <div class="prose prose-earth max-w-none text-earth-600 text-sm leading-relaxed" v-html="faq.answer"></div>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- ¿Aún tienes dudas? -->
    <section class="py-16 bg-gradient-to-r from-primary-50 to-amber-50">
      <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h2 class="text-2xl font-serif font-bold text-earth-900 mb-4">
          {{ pageContent.footer.title }}
        </h2>
        ...
        <NuxtLink
          to="/contact"
          class="inline-flex items-center gap-2 bg-primary-600 hover:bg-primary-700 text-white px-8 py-4 rounded-full text-lg font-semibold transition-all hover:shadow-xl hover:shadow-primary-200"
        >
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
          </svg>
          {{ pageContent.footer.button_text }}
        </NuxtLink>
      </div>
    </section>
  </div>
...
useSeoMeta({
  ...
  ogDescription: 'Todo sobre nuestros productos: cómo comprar, envío Prime 2 días, cómo usar velas y aceites, y más.',
})
...
const pageContent = reactive({
  ...
  prime_banner: {
    ...
    text: 'Todos nuestros productos están disponibles en Amazon México con envío <strong class="text-white">Prime de 2 días máximo</strong>. ¿No tienes Prime? Los envíos estándar llegan en 3 a 5 días hábiles.',
  },
  faqs: [
    {
...
// Alias para el template (listas reactivas)
const faqs = computed(() => pageContent.faqs)
...
async function loadConfig() {
  ...
  try {
    ...
    if (data?.value) {
      ...
      if (data.value.prime_banner) Object.assign(pageContent.prime_banner, data.value.prime_banner)
      if (Array.isArray(data.value.faqs)) pageContent.faqs = data.value.faqs
      if (data.value.footer) Object.assign(pageContent.footer, data.value.footer)
    }
  } catch (e) {
    console.warn('Usando datos estáticos para FAQ (fallback):', e.message)
  }
}
