  ...
  <section class="py-20 bg-gradient-to-r from-primary-900 to-earth-900">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
      <h2 class="text-3xl md:text-4xl font-serif font-bold text-white mb-4">
        {{ effectiveContent.title || '¿Listo para transformar tu hogar?' }}
      </h2>
      <p v-if="effectiveContent.description" class="text-primary-200 text-lg mb-8 max-w-2xl mx-auto">
        {{ effectiveContent.description }}
      </p>
      <!-- Si es un botón de WhatsApp (link wa.me), mostrar botón verde -->
      <a
        v-if="isWhatsAppLink(effectiveContent.button_link)"
        :href="effectiveContent.button_link"
        target="_blank"
      ...
      >
        ...
        {{ effectiveContent.button_text || 'Escríbenos por WhatsApp' }}
      </a>
      <!-- Si es un enlace interno (#), mostrar botón normal -->
      <a
        v-else-if="effectiveContent.button_link && effectiveContent.button_link.startsWith('#')"
        :href="effectiveContent.button_link"
      ...
      >
        {{ effectiveContent.button_text || 'Ver más' }}
      </a>
      <!-- Fallback: botón de Amazon -->
      <a
        v-else
        :href="effectiveContent.button_link || 'https://www.amazon.com.mx/stores/PitayaLab/page/9A7C33BA-7EBF-41E8-9F0F-FEE7FE78A329'"
        target="_blank"
        rel="noopener noreferrer"
...
async function loadDefaults() {
  ...
  const defaults = await useSectionDefaults('cta', null)
  if (defaults) {
    defaultCta.value = defaults
  }
}
