  ...
  <div class="space-y-4">
    ...
    <div v-else-if="section.type === 'products'">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="md:col-span-2">
          <label class="block text-sm font-medium text-gray-700 mb-1">Filtrar por categoría (opcional)</label>
          <select v-model="section.content.category_slug" class="w-full px-3 py-2 rounded-lg border border-gray-200 focus:border-primary-400 outline-none text-sm bg-white">
            <option value="">— Automático (sin filtro) —</option>
            <option v-for="c in categories" :key="c.slug" :value="c.slug">{{ c.name }}</option>
          </select>
          <p class="text-xs text-gray-400 mt-1">Muestra automáticamente los productos activos de esta categoría. Si eliges productos manualmente abajo, esa selección tiene prioridad.</p>
        </div>
